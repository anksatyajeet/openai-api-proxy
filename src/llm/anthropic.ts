      ...
      return {
        ...
        content: await Promise.all(
          it.content.map(async (it) => {
            if (it.type === 'text') {
              return { type: 'text', text: it.text } as AnthropicTypes.TextBlockParam;
            }
...
export function anthropicBase(
  createClient: (apiKey: string) => AnthropicTypes,
): Omit<IAnthropicVertex, 'requiredEnv' | 'supportModels'> {
  return {
    name: 'anthropic',
    async parseRequest(req, apiKey) {
      let r: AnthropicTypes.MessageCreateParamsNonStreaming = {
        ...
        messages: await convertMessages(
          req.messages.filter((it) =>
            ['user', 'assistant'].includes(it.role),
          ),
        ),
    ...
    async invoke(req, apiKey) {
      const client = createClient(apiKey);
      return this.parseResponse(
        await client.messages.create(
          (await this.parseRequest(req, apiKey)) as AnthropicTypes.MessageCreateParamsNonStreaming,
        ),
      );
    },
    async *stream(req, signal, apiKey) {
      const client = createClient(apiKey);
      const stream = await client.messages.create({
        ...(await this.parseRequest(req, apiKey)),
        stream: true,
      });
